...
for(res in c("Smoothed")) {
  for(i in c(1,6,7,9)) {
    ...
    else if(pal_WB[var, "color"]=="Reds") cols = colorRampPalette(c("white","red"))(length(b)-1)
    else if(pal_WB[var, "color"]=="Blues") cols = colorRampPalette(c("white","dark blue"))(length(b)-1)
    else if(pal_WB[var, "color"]=="RdYlBu") cols = colorRampPalette(c("red","yellow","blue"))(length(b)-1)
    ...
    for(j in 1:length(levels(weeks))) {
      ...
      spdf = spdftmp
      png(paste0("GIFs/", res,"/",var,"/",var,"_",j,".png"), width=width, height=height)
      par(mar=c(0.5,0.5,3,0.5))
      image(spdf[var], breaks = b, main=paste0(dw[1], " to ", dw[length(dw)]), col=cols)
      plot(cat.contour, add=TRUE)
      dev.off()
    }
  }
}
