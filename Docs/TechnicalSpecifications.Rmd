...
This service offers model-based estimates of the historic (1986-2016), recent trends (up to 1 year) and projected scenarios (up to year 2100) in soil moisture levels of forests and drought stress of main tree species over Catalonia (NE Spain). Estimates are obtained using a soil plant water balance (SPWB) model and forest plot data of the Spanish National Forest Inventory (De Cáceres et al. 2015). Weather data comes primarily from weather station networks and is processed using the routines described in De Cáceres et al. (2018). Water balance is done at the stand level only, neglecting lateral hydrological processes. The service is intended to complement DEBOSCAT, a monitoring program of observed drought-related forest decline events (mortality, decoloration and leaf loss) conducted in the same area (Chaparro et al. 2016).


### Soil plant water balance (SPWB) model

The water balance model described in De Cáceres et al. (2015) follows the design principles from BILJOU (Granier et al. 1999) and SIERRA water balance submodel (Mouillot et al. 2001). The model performs daily updates of soil water content as a function of the stand structure and daily weather (radiation, temperature and precipitation). Soil water balance is the difference between processes determining water input (i.e. precipitation) and outputs (canopy interception, tree transpiration, bare soil evaporation, surface runoff and deep drainage). Details of the formulation of each of these processes are given in De Cáceres et al. (2015). The model is implemented in the [medfate](http://vegmod.ctfc.cat/medfateweb) R package.


*Weather inputs*

The SPWB model requires values of the following daily weather variables: precipitation, potential evapotranspiration (PET). Average daily temperature and solar radiation are also required, but these variables are not currently stored.
...
Details of weather data sources and processing are given below.

---

*Forest water balance*

Every simulation day, the SPWB model updates the leaf area index (LAI) of forests according to a simple phenology model and produces the following variables describing the water balance in forest stands:
...
SPWB simulations are conducted on two sets of locations:
...
+ Soil depth and root distribution estimated as described in Cabon et al. (2018).
...
Weather station data of the current day is downloaded from the Spanish [Agencia Estatal de Meteorologia (AEMET)](https://opendata.aemet.es/centrodedescargas/inicio) using their [OpenData API](https://opendata.aemet.es). This information, complemented with weather data from previous days, is used to interpolate weather variables over the simulation locations (pixels or IFN3 plots). Precipitation, temperature and relative humidity are interpolated for the target locations and solar radiation is estimated from latitude, topography and the other variables (see details in De Cáceres et al. 2018). Interpolation routines are implemented in the R package *meteoland*. 
...
MODE 3 (projection)| Bias correction & SPWB        |  10 days           | No         | No    |
